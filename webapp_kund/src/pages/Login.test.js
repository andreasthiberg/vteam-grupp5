...
describe('Login component', () => {
    ...
    test('calls setJwt, setUserEmail, and setLoggedIn when logging out', () => {
        props.jwt = 'abc123';

        const { getByText } = render(<Login {...props} />);

        const logoutButton = getByText('Logga ut');
        act(() => {
            logoutButton.click();
        });

        expect(props.setJwt).toHaveBeenCalledWith('');
        expect(props.setUserEmail).toHaveBeenCalledWith('');
        expect(props.setLoggedIn).toHaveBeenCalledWith(false);
    });
});
